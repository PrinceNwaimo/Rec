package africa.semicolon.regcrow.utils;

public class ExceptionUtils {
    public static final String USER_REGISTRATION_FAILED = "Registration failed for user with email %s";
    public static final String USER_WITH_ID_NOT_FOUND = "User with id %d not found";
    public static final String PROFILE_UPDATE_FAILED="Profile Update Failed";
    public static final String FAILED_TO_GET_ACTIVATION_LINK ="Activation link failed";

    public static final String ACCOUNT_VERIFICATION_FAILED ="Account verification failed";
    public static final String USER_WITH_EMAIL_NOT_FOUND = "User not found";

    public static final String AUTHENTICATION_FAILED_FOR_USER_WITH_EMAIL = "Authentication failed";
}

